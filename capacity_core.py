...
def chat_fte_daily(chat_df: pd.DataFrame, settings: dict) -> pd.DataFrame:
    """Daily FTE for Chat using concurrency and utilization.
    Requires columns: ['date','items','aht_sec','program'].
    FTE = (items * aht_sec / concurrency) / (hrs*3600*(1-shrink)*util_chat)
    """
    ...
    util   = float(settings.get("util_chat", settings.get("util_bo", 0.85)) or 0.85)
    conc   = float(settings.get("chat_concurrency", 1.5) or 1.0)
    denom  = hrs * 3600.0 * (1.0 - shrink) * util

    d = chat_df.copy()
    L = {c.lower(): c for c in d.columns}
    date_c = L.get("date")
    items_c= L.get("items") or L.get("volume")
    ...
    out = pd.DataFrame({
        ...
        "aht_sec": pd.to_numeric(d[aht_c], errors="coerce"),
        "program": (d[prog_c].astype(str) if prog_c else "Chat"),
    }).dropna(subset=["date","items","aht_sec"])

    out["work_seconds"] = out["items"] * out["aht_sec"] / max(1e-6, conc)
    g = out.groupby(["date","program"], as_index=False)["work_seconds"].sum()
    g["chat_fte"] = g["work_seconds"] / max(1e-6, denom)
    return g[["date","program","chat_fte"]]
...
def required_fte_daily(voice_df: pd.DataFrame, bo_df: pd.DataFrame, ob_df: pd.DataFrame, settings: dict) -> pd.DataFrame:
    ...
    # Outbound (OPC × Connect Rate × RPC Rate) with talk-time conversion
    def _to_frac(x):
        try:
            v = float(x)
    ...
    if isinstance(ob_df, pd.DataFrame) and not ob_df.empty:
        ...
        # normalize basics
        d[c_date] = pd.to_datetime(d[c_date], errors="coerce").dt.date if c_date else pd.to_datetime(d.get("date")).dt.date
        ...
        if c_conn: d[c_conn]= d[c_conn].map(_to_frac)
        if c_rpc_r: d[c_rpc_r] = d[c_rpc_r].map(_to_frac)

        # aht fallback from settings
        if not c_aht or d[c_aht].isna().all():
            c_aht = c_aht or "_aht_ob_"
            d[c_aht] = float(settings.get("ob_aht_sec", settings.get("target_aht", 240)) or 240)
        else:
            d[c_aht] = pd.to_numeric(d[c_aht], errors="coerce").fillna(float(settings.get("ob_aht_sec", settings.get("target_aht", 240)) or 240))

        # expected handled contacts per row
        def _expected_contacts(r):
            opc = float(r.get(c_opc, 0.0) if c_opc else 0.0)
            rpc_ct = float(r.get(c_rpc, 0.0) if c_rpc else 0.0)
            conn = _to_frac(r.get(c_conn, 0.0)) if c_conn else None
            rpc_rate = _to_frac(r.get(c_rpc_r, 0.0)) if c_rpc_r else None
            ...
            if conn is not None:
                return opc * conn
            return opc  # fall back to dials if rates missing

        d["_contacts"] = d.apply(_expected_contacts, axis=1)
        d["_work_seconds"] = d["_contacts"] * d[c_aht]

        # denom: outbound productive seconds
        hrs    = float(settings.get("hours_per_fte", 8.0) or 8.0)
        shrink = float(settings.get("ob_shrinkage_pct", settings.get("shrinkage_pct", 0.30)) or 0.30)
        util   = float(settings.get("util_ob", 0.85) or 0.85)
        denom  = hrs * 3600.0 * (1.0 - shrink) * util
        ...
        g = d.groupby(["date", c_prog], as_index=False)["_work_seconds"].sum()
        g["ob_fte"] = g["_work_seconds"] / max(1e-6, denom)
