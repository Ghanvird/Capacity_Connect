...
from plan_detail._common import _assemble_bo, _assemble_voice, _blank_grid, _canon_scope, _first_non_empty_ts, _learning_curve_for_week, _load_or_blank, _load_or_empty_bulk_files, _load_or_empty_notes, _load_ts_with_fallback, _parse_ratio_setting
...
def _fill_tables_fixed_monthly(ptype, pid, fw_cols, _tick, whatif=None):
    ...
    shr  = _load_or_blank(f"plan_{pid}_shr",  ["OOO Shrink Hours (#)","Inoffice Shrink Hours (#)","OOO Shrinkage %","Inoffice Shrinkage %","Overall Shrinkage %"], month_ids)
    ...
    for m in month_ids:
        plan_hc = float(att_plan_m.get(m, 0.0))
        act_hc  = float(att_act_m.get(m, 0.0))
    ...
    for m in month_ids:
        if ch_first.lower() == "voice":
            monthly_load = float(fw.loc[fw["metric"] == "Forecast", m] if "Forecast" in fw_rows else vF_vol.get(m, 0.0))
