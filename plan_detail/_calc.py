...
from ._common import _learning_curve_for_week
import json, ast
from ._common import (
    _canon_scope,
    _assemble_voice,
...
def _weekly_hc_step_from_roster(roster: pd.DataFrame, week_ids: list[str], role_regex: str) -> dict[str, int]:
    ...
    eff = (R[c_cur] if c_cur in R else R.get(c_work, pd.Series("", index=R.index))).astype(str).str.strip().str.lower()
    is_prod = eff.eq("production")
    role = R[c_role].astype(str).str.strip().str.lower()
    role_mask = role.str.contains(role_regex, na=False, regex=True)
    X = R[role_mask & is_prod].copy()
    if X.empty:
        return {w: 0 for w in week_ids}
    ...
    X["_psw"] = X.get(c_ps).apply(_week_of) if c_ps in X else None
    X["_tw"]  = X.get(c_term).apply(_week_of) if c_term in X else None
...
# --- imports the function expects to find available in this module/file ---
import math, json, ast, datetime as dt
import pandas as pd, numpy as np
from dash import dash_table
import dash

def _fill_tables_fixed(ptype, pid, fw_cols, _tick, whatif=None):
    # ---- guards ----
    ...
    # ---- FW grid shell ----
    spec = (lambda k: {
        "fw": (["Forecast","Tactical Forecast","Actual Volume","Budgeted AHT/SUT","Forecast AHT/SUT","Actual AHT/SUT","Occupancy"]
               if (k or "").strip().lower().startswith("volume") else
               ["Billable Hours","AHT/SUT","Shrinkage","Training"] if (k or "").strip().lower().startswith("billable hours") else
               ["Billable Txns","AHT/SUT","Efficiency","Shrinkage"] if (k or "").strip().lower().startswith("fte based billable") else
    ...
    for w in week_ids:
        ...
        if "Budgeted AHT/SUT" in fw_rows:
            ...
            bud_val = ((f_voice * s_budget_aht) + (f_bo * s_budget_sut)) / denom if denom > 0 else _first_positive(s_budget_aht, s_budget_sut, default=s_budget_aht)
            fw.loc[fw["metric"] == "Budgeted AHT/SUT", w] = float(bud_val)
        if "Target AHT/SUT" in fw_rows:
            denom = float(f_voice + f_bo)
            tgt_val = ((f_voice * s_target_aht) + (f_bo * s_target_sut)) / denom if denom > 0 else _first_positive(s_target_aht, s_target_sut, default=s_target_aht)
            fw.loc[fw["metric"] == "Target AHT/SUT", w] = float(tgt_val)

    # ---- Occupancy (% in FW grid) ----
    ...
    if isinstance(bB, pd.DataFrame) and not bB.empty: bB["sut_sec"] = float(s_budget_sut)
    ...
    planned_nh_w = {
        w: int(round(planned_nh_w.get(w, 0) * max(0.0, min(1.0,
            ( (_lc_with_wf(_learning_curve_for_week(settings, lc_ovr_df, w), w).get("throughput_train_pct", 100.0) / 100.0) *
    ...
    for w in week_ids:
        if ch_first.lower() == "voice":
            ...
            agents_eff = max(1.0, (agents_prod + nest_eff + sda_eff) * occ_frac_w[w])

            aht = _first_positive(wk_aht_sut_actual.get(w), wk_aht_sut_forecast.get(w), s_target_aht, default=s_target_aht)
            aht = max(1.0, float(aht) * (1.0 + aht_delta / 100.0))  # What-If AHT Δ
            ...
            sut = max(1.0, float(sut) * (1.0 + aht_delta / 100.0))  # What-If SUT Δ
            ...
            if bo_model == "tat":
                ...
                prod_hours = bo_wd * bo_hpd * (1.0 - eff_shr) * util_bo
                handling_capacity[w] = float(agents_eff) * prod_hours * (3600.0 / sut)
            else:
    ...
    for w in week_ids:
        if ch_first.lower() == "voice":
            ...
            aht_sut = max(1.0, float(aht_sut) * (1.0 + aht_delta / 100.0))  # apply What-If to SL too
            n = weekly_voice_intervals.get(w)
            intervals = int(n) if isinstance(n, (int, np.integer)) and n > 0 else intervals_per_week_default
            ...
            agents_eff = max(1.0, (float(projected_supply.get(w, 0.0)) + nest_eff + sda_eff) * occ_frac_w[w])
