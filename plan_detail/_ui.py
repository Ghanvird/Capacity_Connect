...
def _update_roster_by_class(pid: str, class_ref: str, ts, te, ns, ne, ps):
    ...
    if n_before:
        ...
        # ✅ correct key
        save_df(f"plan_{pid}_roster", roster)
    return n_before
...
options_panel = dbc.Offcanvas(
    ...
    dbc.Row([
            dbc.Col(dbc.Button(html.I(className="bi bi-calendar-range"), id="opt-view", color="secondary",
                               className="w-100 mb-2", title="View Between Weeks")),
            dbc.Col(dbc.Button(html.I(className="bi bi-save"), id="opt-saveas", color="secondary",
    ...
    dbc.Row([
            ...
            dbc.Col(dbc.Button(html.I(className="bi bi-arrows-expand"), id="opt-extend", color="secondary",
...
switch_modal = dbc.Modal(id="opt-switch-modal", is_open=False,
                         children=dbc.ModalBody([html.H4("Switch Plan"), html.Div(id="opt-switch-body"),
...
# ...
# add after tabs/card:
html.Div([options_toggle, options_panel, view_modal, saveas_modal, extend_modal, switch_modal, compare_modal])
