...
def page_ops():
    ...
    return dbc.Container([
        ...
        dbc.Card(dbc.CardBody([
            dbc.Row([
                dbc.Col(dcc.Graph(id="ops-line-req-sup", config={"displayModeBar": False}), md=6),
                dbc.Col(dcc.Graph(id="ops-bar-volume", config={"displayModeBar": False}), md=6),
            ], className="g-3"),
            dbc.Row([
                dbc.Col(dcc.Graph(id="ops-pie-channel", config={"displayModeBar": False}), md=4),
                dbc.Col(dcc.Graph(id="ops-col-site", config={"displayModeBar": False}), md=4),
                dbc.Col(dcc.Graph(id="ops-waterfall-gap", config={"displayModeBar": False}), md=4),
            ], className="g-3 mb-3"),
        ])),
        ...
        dbc.Card(dbc.CardBody([
            ...
            dash_table.DataTable(id="ops-table-summary", page_size=10, style_table={"overflowX":"auto"}, style_as_list_view=True)
        ]))
    ], fluid=True)
